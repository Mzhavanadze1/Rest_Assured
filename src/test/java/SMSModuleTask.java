...
public class SMSModuleTask {
    ...
    public void testSMSModel() throws SQLException, ClassNotFoundException {
        ...
        for (int i = 0; i < getSMSRequestModel.size(); i++) {
            ...
            Response response = given().headers("content-type", "application/json").contentType(ContentType.JSON).accept(ContentType.JSON).when().get("http://10.195.105.66:7000/api/Consent?TelNumber=" + getSMSRequestModel.get(i).getTelNumber());

            Assert.assertEquals(response.jsonPath().getInt("data.consentStatusId"), getSMSRequestModel.get(i).getConsent());
